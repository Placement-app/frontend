...
export default function Events() {
  ...
  const load = async () => {
    const send = await fetch("http://192.168.1.35:5000/admin/events_approval", {
      method: "GET",
      mode: "cors",
    });
  ...
  const accept = async (d) => {
    const send = await fetch("http://192.168.1.35:5000/admin/approve_events", {
      method: "POST",
      mode: "cors",
      headers: {
  ...
  const decline = async (cid, title, description, link, content, timeStart, timeEnd, removed) => {
    const send = await fetch("http://192.168.1.35:5000/admin/decline_events", {
      method: "POST",
      mode: "cors",
      headers: {
        'Content-Type': 'application/json'
